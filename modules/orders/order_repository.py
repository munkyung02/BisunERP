...
class OrderRepository:
    ...
    def save_mapping_rule_for_item(self, order_item_id: int, product_id: int) -> int:
        ...
        with self._connect() as connection:
            row = connection.execute(
                """SELECT o.platform, oi.platform_product_name, oi.option_name
                   FROM order_items oi JOIN orders o ON o.id=oi.order_id
                   WHERE oi.id=?""", (int(order_item_id),)
            ).fetchone()
            if row is None:
                raise ValueError("주문상품을 찾을 수 없습니다.")
            connection.execute(
                """INSERT INTO product_mapping_rules
                   (platform, platform_product_name, option_name, product_id, is_active, updated_at)
                   VALUES (?, ?, ?, ?, 1, CURRENT_TIMESTAMP)
                   ON CONFLICT(platform, platform_product_name, option_name) DO UPDATE SET
                     product_id=excluded.product_id, is_active=1, updated_at=CURRENT_TIMESTAMP""",
    ...
    def apply_saved_rule(
        ...
        """저장 규칙을 미매핑 주문상품에 적용합니다.

        order_item_id 또는 order_id가 주어지면 해당 범위만 처리합니다.
        둘 다 없으면 전체 미매핑 주문상품을 대상으로 합니다.
        """
        ...
        with self._connect() as connection:
            rows = connection.execute(
                ...
                INNER JOIN product_mapping_rules AS r
                    ON r.platform = COALESCE(o.platform, '')
                   AND r.platform_product_name = TRIM(
                       COALESCE(oi.platform_product_name, '')
                   )
                   AND r.option_name = TRIM(
    ...
    def delete_mapping_rule_for_item(self, order_item_id: int) -> int:
        with self._connect() as connection:
            row=connection.execute("""SELECT COALESCE(o.platform,''), TRIM(COALESCE(oi.platform_product_name,'')),
                TRIM(COALESCE(oi.option_name,'')) FROM order_items oi JOIN orders o ON o.id=oi.order_id WHERE oi.id=?""",
                (int(order_item_id),)).fetchone()
            if not row: return 0
            cur=connection.execute("DELETE FROM product_mapping_rules WHERE platform=? AND platform_product_name=? AND option_name=?", tuple(row))
    ...
    def get_order_by_id(
        ...
        """
        주문 한 건과 주문상품별 매핑·발주·송장 정보를 조회합니다.

        비선상회 ERP의 업무 종료 지점은 송장등록입니다.
        배송중·배송완료 추적은 주문 상세에서 다루지 않습니다.
        """
        ...
        with self._connect() as connection:
            ...
            item_rows = connection.execute(
                ...
                SELECT
                    ...
                    po.purchase_status AS item_purchase_status,
                    ...
                    sh.courier_name,
                    sh.tracking_number,
                    sh.shipment_status AS saved_shipment_status,
                    sh.shipped_at AS tracking_registered_at,
                ...
                LEFT JOIN purchase_orders AS po
                    ON po.id = (
                        ...
                        WHERE po2.order_item_id = oi.id
                    )

                LEFT JOIN shipments AS sh
                    ON sh.id = (
                        SELECT MAX(sh2.id)
                        FROM shipments AS sh2
                        WHERE sh2.order_item_id = oi.id
                    )
        ...
        result["workflow_status"] = (
            self._build_order_workflow_status(
                result["items"]
            )
        )
    ...
    def _build_order_detail_item(
        ...
        elif item.get("purchase_order_id"):
            item["display_purchase_status"] = (
                "발주완료"
            )
        else:
    ...
    @staticmethod
    def _build_order_workflow_status(
        items: list[dict[str, Any]],
    ) -> str:
        """주문 전체의 현재 업무 단계를 계산합니다."""

        if not items:
            return "주문접수"

        all_tracking_registered = all(
            str(
                item.get("tracking_number") or ""
            ).strip()
            for item in items
    ...
    def auto_map_order_items(
        ...
        """
        저장 규칙, 완전일치, 스마트 점수 추천 순서로 자동 매핑합니다.

        처리 우선순위:
        1. 저장된 영구 매핑 규칙
        2. 정규화된 상품명 완전일치
        3. SmartMappingEngine 자동확정 기준 충족

        자동확정 점수와 후보 간 최소 점수 차이는
        SmartMappingEngine 설정값을 그대로 사용합니다.
        """
        ...
        if order_id is not None:
            ...
            order_condition = "AND oi.order_id = ?"
            parameters.append(valid_order_id)

        # 가장 신뢰도가 높은 저장 규칙을 먼저 적용합니다.
        saved_rule_count = self.apply_saved_rule(
            order_id=order_id,
        )
            ...
            try:
                for row in target_rows:
                    ...
                    for product in product_rows:
                        ...
                        # 상품에 플랫폼이 지정된 경우 주문 플랫폼과 같아야 합니다.
                        if (
                            product_platform
                            and product_platform != order_platform
                        ):
                            continue
                    ...
                    # 완전일치가 없을 때만 스마트 점수 추천을 실행합니다.
                        ...
                        for product in eligible_products:
                            comparison_parts = [
                                ...
                                str(product["option_name"] or "").strip(),
                            ]
                        ...
                        recommendations = (
                            self.mapping_engine.recommend(
                                source_name,
                                engine_products,
                            )
                            if source_name and engine_products
                            else []
                        )
                        ...
                        if recommendations:
                            ...
                            if (
                                top.score
                                >= self.mapping_engine.AUTO_MAPPING_SCORE
                                and top.score - second_score
                                >= self.mapping_engine.AUTO_MAPPING_MIN_GAP
                            ):
                    ...
                    connection.execute(
                        ...
                        (
                            ...
                            selected_product["purchase_round"],
                            row["id"],
                        ),
                    )

                    if mapping_method == "smart":
        ...
        automatic_mapped_count = (
            exact_mapped_count + smart_mapped_count
        )

        return {
            "target_count": len(target_rows),
            "mapped_count": (
                automatic_mapped_count + saved_rule_count
            ),
            "rule_mapped_count": saved_rule_count,
            "exact_mapped_count": exact_mapped_count,
            "smart_mapped_count": smart_mapped_count,
    ...
    def map_order_item(
        ...
        """
        주문상품을 ERP 상품에 연결하고 해당 선택을 영구 매핑 규칙으로 학습합니다.

        수동매핑·추천확정·신규상품매핑처럼 사용자가 확정한 연결은
        product_mapping_rules에 저장되어 같은 플랫폼·상품명·옵션의
        다음 주문부터 자동매핑에 사용됩니다.
        """

        valid_order_item_id = self._validate_id(
            order_item_id,
            "주문상품 ID",
        )
        ...
        with self._connect() as connection:
            try:
                ...
                # 사용자가 확정한 매핑만 영구 규칙으로 학습합니다.
                if valid_mapping_status in {
                    "수동매핑",
                    "추천확정",
                    "신규상품매핑",
                    "매핑완료",
