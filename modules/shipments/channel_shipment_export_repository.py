...
class ChannelShipmentExportRepository:
    ...
    def get_auction_candidates(
        ...
        *,
        ...
        parameters: list[Any] = [self.AUCTION, "배송중"]
        self._add_shipment_id_condition(
            conditions,
            parameters,
            shipment_ids,
    ...
    def count_auction_missing_metadata(
        self,
        *,
        shipment_ids: Iterable[int] | None = None,
    ) -> int:
        ...
        parameters: list[Any] = [self.AUCTION, self.AUCTION, "배송중"]
        self._add_shipment_id_condition(
            conditions,
            parameters,
            shipment_ids,
    ...
    def get_toss_candidates(
        ...
        *,
        ...
        conditions = [
            "m.platform = ?",
            "o.shipment_status = ?",
            "TRIM(COALESCE(m.raw_source_json, '')) != ''",
            "TRIM(COALESCE(sh.courier_name, '')) != ''",
            "TRIM(COALESCE(sh.tracking_number, '')) != ''",
        ]
        parameters: list[Any] = [self.TOSS, "배송중"]
        self._add_shipment_id_condition(conditions, parameters, shipment_ids)
        if shipment_ids is not None and not parameters[2:]:
            return []
    ...
    def count_toss_missing_metadata(
        self,
        *,
        shipment_ids: Iterable[int] | None = None,
    ) -> int:
        conditions = [
            ...
            "TRIM(COALESCE(sh.courier_name, '')) != ''",
            "TRIM(COALESCE(sh.tracking_number, '')) != ''",
            "(m.id IS NULL OR TRIM(COALESCE(m.raw_source_json, '')) = '')",
        ]
        parameters: list[Any] = [self.TOSS, "배송중"]
        self._add_shipment_id_condition(conditions, parameters, shipment_ids)
        if shipment_ids is not None and not parameters[2:]:
