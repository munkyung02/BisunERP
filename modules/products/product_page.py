...
class ProductPage(QWidget):
    ...
    COLUMN_SUPPLIER_COUNT = 7
    COLUMN_MAPPING_COUNT = 8
    COLUMN_PURCHASE_PRICE = 9
    COLUMN_SALE_PRICE = 10
    COLUMN_PURCHASE_ROUND = 11
    COLUMN_STATUS = 12
    COLUMN_NOTION_SYNC = 13
    ...
    def _create_widgets(self) -> None:
        ...
        self.quick_create_button.setObjectName(
            "primaryButton"
        )
        self.quick_create_button.setToolTip(
            "공급처·플랫폼·발주차수를 유지한 채 상품을 연속 등록합니다."
        )

        self.supplier_button = QPushButton(
            "공급처 조건"
        )
        self.supplier_button.setObjectName(
            "secondaryButton"
        )
        ...
        self.template_button.setObjectName(
            "secondaryButton"
        )
        ...
        self.notion_all_button.setObjectName(
            "notionButton"
        )
    ...
    def _create_layout(self) -> None:
        ...
        button_layout.addWidget(
            self.notion_selected_button
        )
        button_layout.addWidget(
            self.notion_all_button
        )
        button_layout.addWidget(
    ...
    def _configure_table_header(self) -> None:
        ...
        self.product_table.setColumnWidth(
            self.COLUMN_SUPPLIER_COUNT,
            85,
        )
        self.product_table.setColumnWidth(
            self.COLUMN_MAPPING_COUNT,
            80,
        )
        self.product_table.setColumnWidth(
    ...
    def _connect_signals(self) -> None:
        ...
        self.template_button.clicked.connect(
            self.save_excel_template
        )

        self.notion_selected_button.clicked.connect(
            self.sync_selected_product_to_notion
        )

        self.notion_all_button.clicked.connect(
            self.sync_all_products_to_notion
        )
    ...
    def _set_product_row(
        ...
        self.product_table.setItem(
            row_index,
            self.COLUMN_SUPPLIER_COUNT,
            self._create_number_item(
                self._to_int(
                    product.get("supplier_count")
                )
            ),
        )
        ...
        sync_status = self._display_text(
            product.get("sync_status")
        ) or "미동기화"
        sync_item = QTableWidgetItem(sync_status)
        sync_item.setTextAlignment(Qt.AlignmentFlag.AlignCenter)
        sync_item.setToolTip(
            self._display_text(product.get("sync_message"))
            or self._display_text(product.get("notion_last_sync"))
        )
        if sync_status == "동기화완료":
            sync_item.setForeground(QColor("#127A45"))
            sync_item.setBackground(QColor("#EAF8F0"))
        elif sync_status == "동기화실패":
            sync_item.setForeground(QColor("#A33A32"))
            sync_item.setBackground(QColor("#FDEEEE"))
        else:
            sync_item.setForeground(QColor("#6B7280"))
            sync_item.setBackground(QColor("#F3F4F6"))
        self.product_table.setItem(
            row_index,
            self.COLUMN_NOTION_SYNC,
            sync_item,
    ...
    def open_quick_create_dialog(self) -> None:
        dialog = ProductDialog(
            parent=self,
    ...
    def open_supplier_dialog(self) -> None:
        ...
        if product_id is None:
            QMessageBox.information(
                self,
                "상품 선택",
                "공급처 조건을 관리할 상품을 선택해 주세요.",
            )
            return

        dialog = ProductSupplierDialog(
            product_id,
            self,
        )
        dialog.exec()
    ...
    def save_excel_template(self) -> None:
        ...
        widths = [16, 12, 34, 24, 18, 18, 28, 14, 14, 12, 12]
    ...
    def import_excel(self) -> None:
        ...
        try:
            ...
            answer = QMessageBox.question(
                self,
                "상품 일괄등록",
                f"엑셀 상품 {len(rows):,}건을 등록하시겠습니까?\n\n"
                "상품코드가 기존 상품과 같으면 수정됩니다.",
            ...
            result = self.repository.import_products(
                rows
            )
        ...
        message = (
            ...
            f"기존 수정: {result['updated_count']:,}건\n"
    ...
    def _read_product_excel(
        ...
        try:
            ...
            aliases = {
                "product_code": {"상품코드", "product_code"},
                "platform": {"플랫폼", "판매처", "platform"},
                "platform_product_name": {
                    "판매처 상품명",
                    "판매처상품명",
                    "platform_product_name",
                ...
                "option_name": {"옵션명", "옵션", "option_name"},
                "supplier_name": {"공급처명", "공급처", "supplier_name"},
                "supplier_product_name": {
                    "공급처 상품명",
                    "공급처상품명",
                    "supplier_product_name",
                },
                "purchase_price": {"매입가", "매입단가", "purchase_price"},
                "sale_price": {"판매가", "판매단가", "sale_price"},
                "purchase_round": {"발주차수", "purchase_round"},
                "is_active": {"사용여부", "상태", "is_active"},
            }
            ...
            for row_number in range(
                ...
                active_text = str(
                    value("is_active") or "사용"
                ).strip().lower()

                is_active = 0 if active_text in {
                    "중지",
                    "미사용",
                    "0",
                    "false",
                ...
                rows.append({
                    ...
                    "platform_product_name": value("platform_product_name"),
                    "product_name": product_name,
                    "option_name": value("option_name"),
                    "supplier_name": value("supplier_name"),
                    "supplier_product_name": value("supplier_product_name"),
                    "purchase_price": self._to_int(value("purchase_price")),
                    "sale_price": self._to_int(value("sale_price")),
                    "purchase_round": value("purchase_round"),
                    "is_active": is_active,
                })
    ...
    def sync_all_products_to_notion(self) -> None:
        answer = QMessageBox.question(
            self,
            "전체 상품 Notion 동기화",
            "ERP의 전체 상품을 Notion 상품 DB에 업로드하거나 갱신하시겠습니까?\n\n"
            "현재 단계는 ERP → Notion 단방향 동기화입니다.",
    ...
    def _run_notion_sync(self, operation: Any) -> None:
        self._set_loading_state(True)
        self.notion_selected_button.setEnabled(False)
        self.notion_all_button.setEnabled(False)
        self.result_count_label.setText("Notion 상품 DB와 동기화하는 중입니다...")
        try:
            result = operation()
        except Exception as error:
            QMessageBox.critical(
                self,
                "Notion 동기화 오류",
                "상품을 Notion에 동기화하지 못했습니다.\n\n"
                f"{error}",
            )
            return
        finally:
            self._set_loading_state(False)
            self.notion_all_button.setEnabled(True)

        errors = list(result.get("errors") or [])
        message = (
            f"대상: {int(result.get('target_count', 0)):,}건\n"
            f"신규 생성: {int(result.get('created_count', 0)):,}건\n"
            f"기존 갱신: {int(result.get('updated_count', 0)):,}건\n"
            f"실패: {int(result.get('failed_count', 0)):,}건"
        )
        if errors:
            preview = "\n".join(
                f"· {item.get('product_name')}: {item.get('error')}"
                for item in errors[:5]
            )
            message += f"\n\n실패 일부\n{preview}"
    ...
    def _set_loading_state(
        ...
        self.notion_all_button.setEnabled(
            not loading
        )
        if loading:
            self.notion_selected_button.setEnabled(False)
